...
class SeatGrid {
	...
	required init(_ inputLines: [String]) {
		self.grid = CharGrid(inputLines: inputLines)
	...
	func pointsOfInterest(_ x: Int, _ y: Int) -> [GridPoint] {
		return grid.neighboringPoints(of: GridPoint(x, y))
	}
	...
	func numOccupiedSeats(amongPoints points: [GridPoint]) -> Int {
		let valuesAtTheGivenPoints = points.map { grid[$0] }
	...
	static func solve(_ inputLines: [String]) -> String {
		var grid1 = Self(inputLines)
		var grid2 = Self(inputLines)
